...
const LESSON_GUIDE_SEED: Record<string, { teacherGuideMd: string; studentSummary: string }> = {
  lt_module_a_keeper_quest: {
    studentSummary:
      "Срочный корм для единорогов (setosa); кто-то подпортил подписи; две мини-студии в Nodly; в методичке — как подставить готовый проект во вторую мини-студию.",
    teacherGuideMd: `# Методичка: квест «Срочный корм: Iris setosa» (мини-студия)
...
## Готовый проект во **второй** мини-студии (показать образец)
По умолчанию у блока \`q09b_studio\` режим **пустой проект** (\`studioPracticeKind: "empty"\`). Чтобы ученик сразу видел **готовую цепочку** и менял её под себя:

1. В **полной разработке** соберите эталонный проект (данные, блоки), **сохраните в облако**, скопируйте **id проекта** из URL или списка проектов.
2. В **админке** шаблона урока отредактируйте блок **studio** с id \`q09b_studio\`: выставьте **копию облачного проекта** (\`studioPracticeKind: "project_clone"\`) и поле \`referenceProjectId\` = id образца (в редакторе блоков урока — режим «копия проекта»).
3. При первом запуске второй мини-студии ученик получит **снимок-копию**; дальше работает со **своим** облачным проектом, привязанным к этому блоку.
    ...
    blocks: [
      {
        ...
        type: "text",
        textScale: "lg",
        body: `# Срочная партия для единорогов
...
В оранжерее растут **ирисы**. Их **три вида**, и внешне они похожи. Но для **корма единорогов** по рецепту подходят **только** цветки одного вида — **Iris setosa**. Если в смесь попадёт другой вид, корм **не подойдёт**, а единороги **останутся голодными**. Партия нужна **сегодня**.
...
Дальше в уроке всё это будет выглядеть как **привычная таблица** (каждая строка — один цветок, в конце строки — вид). Сейчас важно другое: мы научим **программу** в **Nodly** угадывать вид по четырём числам на строку и так **отберём настоящую setosa** для корма.

Потом коротко про ИИ и данные; в конце — две **мини-студии** в Nodly: **обучить и сохранить модель**, затем **прогнать «битый» лабораторный список** и отметить строки, где программа уверенно видит **setosa**.`
      },
      {
        id: "q00_hero",
        type: "media",
        ...
        caption: "Зал Агентства: Нодус открывает дело о срочной партии корма для единорогов."
      },
      {
        id: "q00_greenhouse",
        type: "media",
...
**В зале** Нодус держит дело в порядке: что спросили, что ответили, какие шаги были. **В оранжереи** — живые цветы и срочность: нужно отобрать **только setosa** в корм.
...
3. Отберём строки, где по факту **setosa** — это и есть **сырьё для корма** единорогам.
...
Наш случай — самый прозрачный: на входе **четыре числа про цветок**, на выходе нужно сказать **один из трёх видов**. Это и есть помощь с **кормом для единорогов**, когда **надпись в журнале врать не должна**, а иногда врёт.`
      },
      {
        id: "q01_ai_examples",
        type: "text",
...
Не «думает как человек» — находит **математические связи**. В нашем сюжете это как раз то, что отличает **настоящую setosa** от обмана в последнем столбце.`
      },
      {
        id: "q02_models_tasks",
        type: "text",
...
Здесь нужна **классификация**: три вида ириса, а в корм пускаем только **setosa**.`
      },
      {
        id: "q02_model_types",
        type: "text",
...
Важно помнить цикл: **данные → обучение → проверка → предсказание** — и дальше решение, **можно ли класть эту строку в корм**.`
      },
      {
        id: "q02_check",
        type: "checkpoint",
        question: "Какую задачу мы решаем для партии корма?",
        answerMode: "single",
...
**Лабораторный файл** — измерения из оранжереи для **корма**: там **нельзя слепо верить** колонке «вид», потому что **кто-то** её испортил. Модель по признакам говорит, **какой вид у цветка на самом деле**.
...
Три вида: **setosa**, **versicolor**, **virginica**. Для **корма единорогов** нужна только **setosa** — остальные в этот рецепт **не входят**. В лабораторной выгрузке после того как **кто-то** подпортил таблички и файл, часть меток **лжёт** или пустая — ориентируйся на **числа** и на модель, обученную на **честном** Iris.`
      },
      {
        ...
        caption: "Панель досье: три вида и четыре признака — шпаргалка перед отбором setosa в корм."
      },
      {
        id: "q04_table",
        type: "text",
...
Ниже — картинка, как делят таблицу на train и test.`
      },
      {
        ...
        caption: "Часть честных примеров — в обучение, часть — в проверку: так честнее оценить модель перед кормом."
      },
      {
        id: "q05_split_text",
        type: "text",
...
Перед **срочной** отгрузкой корма модель может **уверенно** ошибаться. **Метрики** показывают, насколько она хороша на самом деле, а не «на словах».`
      },
      {
        id: "q06_metrics_text",
        type: "text",
...
Если модель **переучилась** только на учебных строках, на **лабораторном** файле из оранжереи она может **промахнуться** — а это уже не шутки: в корм могут попасть **не те** лепестки.
...
Инструменты стажера — **блоки**: цепочка **обучить → предсказать → если setosa — показать**, без кода с нуля. Так быстрее закрыть **срочную** партию для единорогов.`
      },
      {
        id: "q08_blocks_image",
        type: "media",
...
- **Оранжерея лаборатории** — список из оранжереи: по нему потом **прогоняем предсказание** и ищем строки, где по факту **setosa** (в корм). Удобно взять **весь файл** сразу; ответы смотри на вкладке **«Визуализация»** и на **сцене** мини-студии подсказками.`
      },
      {
        id: "q09_lab_title",
        type: "text",
...
**Зачем два раза.** В первом окне ты **учишь** модель на Iris и **сохраняешь** её в библиотеку внутри этого проекта. Во втором открывается **новый пустой проект** (как будто ты сел за другой компьютер): модель из первого окна **сама не перелетает** — поэтому там нужно **коротко обучиться на Iris ещё раз**, а затем уже **прогнать лабораторный файл**, поставить **если … то (без иначе)** и показать сообщение для **Iris-setosa** (это и есть кандидаты в корм, пока подписи в файле врут).

Если учитель заранее подключил **готовый пример** ко второму окну, ты увидишь уже собранную цепочку — можно менять под себя; если открылось пусто, собери по шагам из инструкции ниже.

Справа на **сцене** мини-студии — короткие подсказки и график обучения; длинное чтение — только в ленте слева.`
      },
      {
        id: "q09_lab_image",
        type: "media",
      ...
      { id: "q_div_practice_1", type: "divider" },
      {
        id: "q09_page1_title",
        type: "text",
        body: `## Мини-студия — шаг 1: обучение и сохранение

1. Нажми **«Запустить мини-студию»** — откроется окно с блоками.  
2. В **«Данные» → «Обучение»** выбери **Iris (Iris.csv)**.  
3. Собери цепочку: **Старт → Обучить модель** (тип **таблица: классификация**) → **Сохранить модель в библиотеку**.  
4. Нажми **Старт**, дождись конца обучения, затем ещё раз **Старт**, чтобы выполнилось сохранение.  
5. На **сцене** справа смотри кривую **loss** во время обучения.`
      },
      {
        id: "q09a_studio",
        type: "studio",
        instruction:
          "Шаги: **Старт** → **Обучить модель** (датасет **Iris.csv**, тип **таблица: классификация**) → **Сохранить модель в библиотеку**. Первый **Старт** — обучение; второй **Старт** после него — сохранение. На сцене — график loss и точность.",
        studioPracticeKind: "empty",
        studioWorkspaceLevel: 1,
        goals: [
      ...
      { id: "q_div_practice_2", type: "divider" },
      {
        id: "q09_page2_title",
        type: "text",
        body: `## Мини-студия — шаг 2: лаборатория и условие

1. Прокрути ленту и нажми **«Запустить мини-студию»** ещё раз — это **новое** окно, без цепочки из шага 1.  
2. Снова в **«Данные» → «Обучение»** подключи **Iris** и **Оранжерея лаборатории**.  
3. Собери: **Старт → Обучить модель** (Iris) → **Предсказать** (источник — **весь файл** лаборатории) → **если … то (без иначе)** → **показать сообщение** или **показать результат**, если предсказанный класс **Iris-setosa** (лепестки в корм).  
4. Список ответов — внизу **сцены** после графика.

Почему снова «Обучить модель»: в этом окне свой проект, связь с первым шагом только **смысловая** — как в жизни взяли другой сеанс.`
      },
      {
        id: "q09b_studio",
        type: "studio",
        instruction:
          "**Старт** → **Обучить модель** (Iris) → **Предсказать** (лаборатория, **весь файл**) → **если** класс = **Iris-setosa** → **сообщение** / **результат**. На сцене — график при обучении.",
        studioPracticeKind: "empty",
        studioWorkspaceLevel: 1,
        goals: [
...
- На **Iris** учим честную связь «числа → вид»; лабораторный файл — только для **предсказаний** и отбора **setosa** в корм; файлы в одном шаге не смешивай.  
...
- Учитель может включить **готовый пример** во втором окне (копия проекта) — тогда цепочка уже есть, остаётся понять и подправить.`
      },
...
По цепочке в Nodly мы отделили **настоящую Iris setosa** от обмана в колонке «вид» — **корм для единорогов** снова можно собирать по правилам.

- ИИ учится на данных, не «магия».
- Нужна честная разметка для учёбы и **честная** проверка.
...
**Дело 002** — уже про картинки, не таблицы.`
      },
      {
        ...
        caption: "Архив после срочной партии: корм снова под контролем."
      },
      {
        id: "q10_final_hook",
        type: "text",
...
**Дело 002: изображения** — модель учится по картинкам, не по столбцам таблицы. Истории с **битыми подписями** и **чужим вмешательством** всплывают и там — привыкай проверять цепочку **данные → обучение → проверка → предсказание**.`
      },
      {
        id: "q10_final_check",
        type: "checkpoint",
  ...
  await prisma.lessonTemplate.upsert({
    ...
    create: {
      ...
      title: "Срочный корм: Iris setosa",
      description: "Единороги, корм только из setosa; кто-то поломал надписи в оранжерее; первая модель в Nodly.",
      moduleKey: "module_a",
      sortOrder: 0,
      starterPayload: EMPTY_SNAPSHOT,
      ...
      studentSummary: "Срочная партия корма для единорогов: только setosa; кто-то испортил метки; Iris и лабораторный файл; две мини-студии в Nodly."
    },
    update: {
      title: "Срочный корм: Iris setosa",
      description: "Единороги, корм только из setosa; кто-то поломал надписи в оранжерее; первая модель в Nodly.",
