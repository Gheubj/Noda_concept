...
import { IRIS_QUEST_LESSON_BLOCKS } from "./irisQuestLessonBlocks.js";
...
const studioBlockZ = z.object({
  ...
  reuseMiniDevFromBlockId: z.string().min(1).max(80).optional(),
    ...
    async (req: AuthenticatedRequest, res) => {
      ...
      const body = z
        .object({
          ...
          reuseProjectId: z.string().min(1).optional()
        })
        .safeParse(req.body);
      if (!body.success) {
      ...
      const reuseId = body.data.reuseProjectId?.trim();
      if (reuseId) {
        const existing = await prisma.project.findFirst({
          where: { id: reuseId, userId: req.session!.sub },
          select: { id: true, lessonTemplateId: true }
        });
        if (!existing) {
          res.status(400).json({ error: "Проект для продолжения не найден" });
          return;
        }
        if (existing.lessonTemplateId !== lessonId) {
          res.status(403).json({ error: "Проект не относится к этому уроку" });
          return;
        }
...
export async function ensureModuleAQuestTemplate() {
  ...
  const questContent: Prisma.InputJsonValue = {
    schemaVersion: 2,
    blocks: IRIS_QUEST_LESSON_BLOCKS as unknown as Prisma.InputJsonValue,
    presentationPdfUrl: null,
    slides: [],
    practiceSteps: [],
    checkpoints: [],
