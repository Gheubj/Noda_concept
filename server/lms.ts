...
В деле только **таблица**: четыре числа на цветок — длины и ширины лепестка и чашелистика. Фотографии нет, как в учебной задаче. Вопрос простой: **можно ли по этим числам понять вид ириса**?

От ответа ясно, зачем здесь Iris и зачем в сюжете фигурируют пигмент и пыльца.`
      },
      {
        id: "q00_hypothesis",
        type: "checkpoint",
...
Типичная цепочка: **Старт → Обучить модель → Предсказать → (если нужно) если … то → показать сообщение / результат**.`
...

      {
        ...
        type: "text",
        textScale: "lg",
        body: `## Сцена 9. Типы моделей для таблицы
...
**Iris (Iris.csv)** — учимся. **Оранжерея лаборатории** — проверяем предсказаниями (одна строка или **весь файл**; список — на вкладке «Визуализация» и на «Сцене»).`
      },
      {
        id: "q09_lab_title",
        type: "text",
        textScale: "lg",
        body: `## Сцена 10. Две мини-разработки

**Страница 1** — обучить классификатор на Iris и **сохранить модель** в библиотеку. **Страница 2** — новый черновик среды: снова коротко **обучи на Iris**, затем **Предсказать** по лабораторному файлу (лучше **весь файл**), **если … то** и сообщение/результат для нужного вида (как в деле про пигмент).

На «Сцене» короткие подсказки и график обучения; длинный текст — только в ленте урока слева.`
      },
      {
        id: "q09_lab_image",
        type: "media",
        kind: "image",
        url: "/api/coach/idle.png",
        caption: "Нодус в Архиве — дальше две вкладки мини-студии по очереди."
      },
      { id: "q_div_practice_1", type: "divider" },
      {
        id: "q09_page1_title",
        type: "text",
        body: `## Страница 1. Обучение и сохранение

Собери цепочку: **Старт → Обучить модель** (Iris, классификация) → **Сохранить модель в библиотеку**. Запусти **Старт** и дождись конца обучения, затем снова **Старт**, чтобы выполнилось сохранение. На «Сцене» смотри кривую **loss** во время обучения.`
      },
      {
        id: "q09a_studio",
        type: "studio",
      ...
      { id: "q_div_practice_2", type: "divider" },
      {
        id: "q09_page2_title",
        type: "text",
        body: `## Страница 2. Лаборатория и условие

Открылась **вторая** мини-студия (новый облачный черновик): модель из шага 1 здесь не переносится — как после переустановки. **Снова** обучи на **Iris**, затем **Предсказать** по **Оранжерея лаборатории** (**весь файл**), добавь **если … то (без иначе)** и **показать сообщение** или **показать результат** для выбранного вида (например Iris-versicolor). Список ответов — внизу «Сцены» после графика.`
      },
      {
        id: "q09b_studio",
        type: "studio",
        instruction:
          "**Старт** → **Обучить модель** (Iris) → **Предсказать** (лаборатория, **весь файл**) → **если** предсказанный класс = нужный вид → **показать сообщение** / **результат**. Смотри график при обучении.",
        studioPracticeKind: "empty",
        studioWorkspaceLevel: 1,
        goals: [
