...
const studioBlockZ = z.object({
  ...
  stageInstruction: z.string().max(2000).optional().nullable(),
...
- Полный экран из меню **«Разработка»** называем **полной разработкой** и отделяем от урока: там личные проекты, здесь — только задача квеста.

## Готовый проект во **второй** мини-студии
По умолчанию у блока \`q09b_studio\` режим **пустой проект** (\`studioPracticeKind: "empty"\`). Варианты в админке у блока Studio:

1. **Пустая практика** — ученик собирает цепочку с нуля.
2. **Снимок из шаблона урока** (\`studioPracticeKind: "template"\`) — если в шаблоне заполнен **starterPayload** с готовой цепочкой.
3. **Копия облачного проекта** (\`studioPracticeKind: "project_clone"\`, поле \`referenceProjectId\`) — эталон из **полной разработки** (URL \`/studio?project=…\`).

Уровень Blockly для копии берётся из проекта-образца.

## Тайминг (ориентир)
1. Сюжет и мотивация (7 мин).
2. ИИ и данные (15 мин).
3. Train/test и метрики (15 мин).
4. Две мини-студии подряд (20 мин).
5. Финал: **итоги** + **два вопроса на итоги** (без отдельного «слайда» с картинкой и без обещания «картинок»); в конце — **до встречи**: следующий раз подробно **регрессия**, сюжет про **моллюсков**, датасет с **размером**.

## Подсказки
- Если класс путается: «первая мини-студия учит и сохраняет, вторая — заново учит и гоняет лабораторный файл» — это специально имитирует «новую сессию» без переноса модели.
`
  },
  lt_intro_ai: {
    studentSummary:
...
**Нодус** — твой наставник в этом уроке: подскажет шаги и проверит логику.

### Сначала — сюжет, без формальностей

В оранжерее растут **ирисы**. Их **три вида**, и внешне они похожи. По **спецификации партии** в производство допускается **только** **Iris setosa** — остальные виды в эту партию **не входят**. Если в смесь попадёт другой вид, партия **не пройдёт контроль качества**. Срок — **сегодня**.

**Кто-то** побывал в оранжерее: **разбил часть табличек** над грядками и подпортил **запись в журнале** — там, где рядом с цветком должно быть написано, **какой это вид**, теперь **нельзя верить написанному**: где-то пусто, где-то стоит не то имя.

Хорошая новость: лаборанты как обычно **сняли размеры** листьев и лепестков **линейкой** и занесли **числа** в файл. Эти числа **не переписывали** — значит, по ним ещё можно понять, **какой вид на самом деле**, если не смотреть на сломанную подпись.

Дальше в уроке всё это будет выглядеть как **привычная таблица** (каждая строка — один цветок, в конце строки — вид). Сейчас важно другое: мы научим **программу** в **Nodly** угадывать вид по четырём числам на строку и так **отберём строки, где по факту setosa** — как требует спецификация.

Потом коротко про ИИ и данные; в конце — две **мини-студии** в Nodly: **обучить и сохранить модель**, затем **прогнать лабораторный список** и отметить строки, где программа уверенно видит **setosa**.`
      },
      {
        id: "q00_hero",
        type: "media",
        ...
        caption: "Зал Агентства: Нодус открывает лабораторное дело о партии ирисов и подпорченных метках."
      },
      {
        id: "q00_greenhouse",
        type: "media",
        ...
        caption: "Оранжерея: три вида ирисов похожи глазу, а подписи на грядках и в файле после визита кого-то больше не бьют с реальностью."
      },
      {
        id: "q00_story_lab",
        type: "text",
...
**В зале** Нодус держит дело в порядке: что спросили, что ответили, какие шаги были. **В оранжереи** — живые цветы и срочность: нужно отобрать **только setosa** по спецификации.

Подпись «какой это вид» сейчас **подозрительна**. Зато **четыре числа-замера** на цветок остались **как были**. Дальше в уроке мы как раз научимся **не верить сломанной подписи**, а сверяться с **числами и программой**.`
      },
      {
        id: "q00_problem",
        type: "text",
        body: `## Что сделаем дальше (простыми словами)

1. На **честном списке** цветков (файл **Iris.csv**) у каждой строки **и числа, и правильный вид** — на нём **научим программу** в Nodly: «по этим четырём числам скажи вид».

2. Потом прогоним **лабораторный список** из оранжереи: там числа **нормальные**, а вот **написанный вид** — нет. Программа скажет, **кто цветок по факту**.

3. Отберём строки, где по факту **setosa** — это и есть **строки, которые можно включать в партию** по спецификации.

Слово **«классификация»** дальше будет значить ровно это: **по числам выбрать один из трёх видов**. Файлы **Iris.csv** и **Оранжерея лаборатории** подключишь в Nodly в разделе **«Данные»** — к блокам в цепочке вернёмся, когда дойдём до **мини-студии**.`
      },
...

      {
        id: "q01_gate_text",
        type: "text",
        body: `## С чего начинаем

В этом уроке **нет фотографий цветков** — только **четыре числа на один цветок**, как в журнале измерений. Так устроено учебное дело: зато одинаково для всех.

Вопрос, с которого всё строится: **если глазами цветок не смотреть, можно ли по этим четырём числам понять вид** — и отделить **настоящую setosa** от строк, где в конце написано **не то**, потому что **кто-то** подпортил запись?`
      },
      {
        id: "q00_hypothesis",
        type: "checkpoint",
        question: "Если у цветка есть только четыре числа-замера (без фото), можно ли вообще отличить вид?",
        answerMode: "single",
...
**ИИ** в обычной жизни — это не «робот с душой», а **программа**, которая на примерах учится **ловить закономерность**: если на входе есть сигнал (цифры, звук, картинка), на выходе — **ответ** (класс, текст, рекомендация).

Наш случай — самый прозрачный: на входе **четыре числа про цветок**, на выходе нужно сказать **один из трёх видов**. Это как раз то, что нужно, когда **колонка «вид» в журнале врать не должна**, а иногда врёт.`
      },
      {
        id: "q01_ai_examples",
        type: "text",
...
Если по сигналу можно понять «кто / что это», то и **вид ириса по четырём числам** — не чудо, а задача на закономерность.`
      },
      {
        id: "q01_ai_or_not",
        type: "checkpoint",
...
Не «думает как человек» — находит **математические связи**. В нашем деле это как раз то, что отличает **настоящую setosa** от обмана в последнем столбце.`
      },
      {
        id: "q02_models_tasks",
        type: "text",
...
Здесь нужна **классификация**: три вида ириса, а по спецификации партии допускается только **setosa**.`
      },
      {
        id: "q02_model_types",
        type: "text",
...
Важно помнить цикл: **данные → обучение → проверка → предсказание** — и дальше решение, **можно ли относить строку к допустимой партии**.`
      },
      {
        id: "q02_check",
        type: "checkpoint",
        question: "Какую задачу мы решаем для контроля партии ирисов?",
        answerMode: "single",
...
Без данных модель не учится. С данными плохого качества — учится плохо: в нашем деле **кто-то** как раз «подпортил» **метки**, но не обязательно **числа**.
...
Пока без разбора кнопок: ниже — что такое **датасет** и **вход**, это пригодится и в Архиве, и в Nodly. В этом деле в «Данные» попадут и честный **Iris**, и **лабораторный** файл после оранжереи.`
      },
      {
        id: "q03_datasets_io",
        type: "text",
...
**Датасет** — таблица примеров (например Iris): в каждой строке **четыре числа** — измерения чашелистика и лепестка (длина/ширина), это **признаки**; последний столбец — **метка** (вид цветка). В **учебном** Iris метки **честные** — на них учимся.

**Лабораторный файл** — измерения из оранжереи для **этой партии**: там **нельзя слепо верить** колонке «вид», потому что **кто-то** её испортил. Модель по признакам говорит, **какой вид у цветка на самом деле**.
...
Если метка врёт — модель учится **не тому**. В оранжерее **кто-то** сделал ровно это с колонкой «вид».`
      },
      {
        id: "q03_data_check",
        type: "checkpoint",
...
Три вида: **setosa**, **versicolor**, **virginica**. По **спецификации** в текущую партию входит только **setosa** — остальные виды **не подходят**. В лабораторной выгрузке после того как **кто-то** подпортил таблички и файл, часть меток **лжёт** или пустая — ориентируйся на **числа** и на модель, обученную на **честном** Iris.`
      },
      {
        ...
        caption: "Панель досье: три вида и четыре признака — шпаргалка перед отбором setosa по спецификации."
      },
      {
        id: "q04_table",
        type: "text",
...
Строка = пример; числа = **признаки**; вид = **метка**; задача = классификация.`
      ...
      },
      {
        ...
        type: "checkpoint",
        question: "Если лепесток очень короткий, какой вид чаще всего ожидаем в честных данных?",
        answerMode: "single",
...
Чтобы модель не подстраивалась под **лживые** метки из оранжереи, учим её на **честной** таблице Iris и делим строки на **обучение** и **проверку**.
...
Ниже — картинка, как делят таблицу на train и test.`
      },
      {
        ...
        caption: "Часть честных примеров — в обучение, часть — в проверку: так честнее оценить модель перед решением по партии."
      },
      {
        id: "q05_split_text",
        type: "text",
...
Перед **срочной** проверкой партии модель может **уверенно** ошибаться. **Метрики** показывают, насколько она хороша на самом деле, а не «на словах».`
      },
      {
        id: "q06_metrics_text",
        type: "text",
...
Если модель **переучилась** только на учебных строках, на **лабораторном** файле из оранжереи она может **промахнуться** — а это уже не шутки: в партию могут попасть **не те** строки.
...
Дальше всё собирается в **мини-студии** — это Nodly **внутри урока**: палитра блоков, раздел **«Данные»** и кнопка **«Старт»**. Большой экран **«Разработка»** в меню — отдельно: там свои проекты, сейчас он не обязателен.

Инструменты стажера — **блоки**: цепочка **обучить → предсказать → если нужный вид — показать**, без кода с нуля. Так быстрее закрыть **срочную** проверку партии.`
      },
      {
        id: "q08_blocks_image",
        type: "media",
...
Картинки и другие режимы — в других уроках и уровнях. Здесь важно: для вида ириса по столбцам берём **таблица: классификация** — ответом будет **вид**, а не число.`
      },
      {
        id: "q08_models_check",
        type: "checkpoint",
...

      {
        id: "q09_two_datasets",
        type: "text",
        body: `### Два файла в мини-студии («Данные → Обучение»)

В левой ленте урока ты читаешь объяснение; в **мини-студии** справа — рабочее окно. Таблицы подключаются **не блоком**, а в боковом разделе **«Данные»** → **«Обучение»**:

- **Iris (Iris.csv)** — «честный» список: на нём **учим** программу отличать три вида.
- **Оранжерея лаборатории** — список из оранжереи: по нему потом **прогоняем предсказание** и ищем строки, где по факту **setosa** (по спецификации партии). Удобно взять **весь файл** сразу; ответы смотри на вкладке **«Визуализация»** и внизу **сцены** после графика.`
      },
      {
        id: "q09_lab_title",
        type: "text",
        textScale: "lg",
        body: `## Две мини-студии подряд

Сейчас будет **два отдельных окна мини-студии** — одно за другим. Каждое открывается кнопкой **«Запустить мини-студию»** под инструкцией: сначала первое задание, дошёл до конца — листай ленту дальше и открой **второе**.

**Зачем два раза.** В первом окне ты **учишь** модель на Iris и **сохраняешь** её в библиотеку внутри этого проекта. Во втором открывается **новый проект** (часто пустой): модель из первого окна **сама не переносится** — поэтому там снова нужно **кратко обучиться на Iris**, затем **прогнать лабораторный файл**, добавить ветку **«если … то (без иначе)»** и вывести результат для строк, где по модели вид **Iris-setosa** (именно они нас интересуют при подпорченных подписях).

**Вторую** мини-студию учитель может настроить в админке: **пустой проект**, **снимок из шаблона урока** или **копия готового облачного проекта** — тогда цепочка может уже открыться собранной, и останется разобрать и подправить.`
      },
      {
        id: "q09_lab_image",
        type: "media",
        kind: "image",
        url: "/api/coach/iris-quest-greenhouse.png",
        caption: "Оранжерея и лабораторный список; дальше — две мини-студии по очереди (кнопка под каждой инструкцией)."
      },
      { id: "q_div_practice_1", type: "divider" },
      {
        id: "q09_page1_title",
        type: "text",
        body: `## Мини-студия — шаг 1

Нужно в **мини-студии** подключить **табличные данные для обучения**, собрать цепочку: **обучение классификатора** и **сохранение модели в библиотеку**, затем **дважды** запустить сценарий кнопкой **«Старт»** так, чтобы сначала отработало обучение, а после него — сохранение.

На **сцене** смотри **график обучения** и **метрики** — по ним понятно, не «застряла» ли модель. Конкретные названия блоков и порядок — в развёрнутой инструкции ниже (по желанию).`
      },
      {
        id: "q09_page1_reveal",
        type: "text",
...
### Развёрнуто: сцена и метрики

- **Loss (ошибка)** — насколько предсказание далеко от правильного ответа **во время обучения**; обычно ждут, что кривая **снижается** от прогона к прогону.
- **Точность (accuracy)** — доля верных ответов на выбранной выборке; удобно сверять **train** и **test**, если они показываются отдельно.
- **Уверенность (confidence)** — насколько модель «уверена» в ответе; высокая уверенность **не заменяет** проверку по accuracy на отложенных строках.

График на сцене привязан к запуску **«Старт»**: после первого старта смотри, как ведёт себя обучение; после второго — что показывает сохранение (если цепочка собрана верно).

**Порядок блоков (напоминание):** **Старт** → **Обучить модель** (тип **таблица: классификация**, датасет **Iris.csv** из «Данные → Обучение») → **Сохранить модель в библиотеку**. Первый **Старт** — обучение; следующий **Старт** — чтобы выполнилось сохранение после успешного обучения.`
      },
      {
        id: "q09a_studio",
        type: "studio",
        instruction:
          "Собери цепочку: обучение **табличной классификации** на честном наборе и **сохранение модели** в библиотеку. Запускай **Старт**, когда цепочка готова; на сцене смотри **график** и **метрики**.",
        stageInstruction:
          "**Сцена:** после **Старт** — график **loss** и показатели качества. Собери цепочку и запускай.",
        studioPracticeKind: "empty",
        studioWorkspaceLevel: 1,
        goals: [
          { id: "q9a_goal_dataset", title: "Подключи табличные данные для обучения", type: "select_dataset", datasetKind: "tabular" },
          { id: "q9a_goal_train_block", title: "Добавь в цепочку обучение классификатора", type: "add_block", blockType: "noda_train_model_simple" },
          { id: "q9a_goal_save_block", title: "Добавь сохранение модели в библиотеку", type: "add_block", blockType: "noda_save_model" },
      ...
      { id: "q_div_practice_2", type: "divider" },
      {
        id: "q09_page2_title",
        type: "text",
        body: `## Мини-студия — шаг 2

Открой **вторую** мини-студию — это **новое** окно. Снова подключи **честный Iris** и **лабораторный список** в **«Данные → Обучение»**, затем собери цепочку: **обучение**, **предсказание по всему лабораторному файлу**, ветка **«если … то (без иначе)»** и **вывод**, когда предсказанный класс совпадает с **Iris-setosa**.

### Как читать такую программу (как на учебной схеме)

Типичная цепочка выглядит так: **Старт** → **Обучить модель** (режим **таблица: классификация**, обучающий файл — например **Iris.csv**) → **Предсказать** (источник — **весь** другой табличный файл, в нашем случае лабораторный список) → **если … то (без иначе)** — в условии сравнивают **предсказанный класс** с нужной **текстовой меткой класса** → внутри ветки **показать результат** или **сообщение**.

На демонстрационных скринах в условии иногда берут, например, **Iris-versicolor** — это просто пример **ветвления**. **В этом задании** порог в «если» должен быть **Iris-setosa**: так мы отмечаем строки, которые по признакам похожи на setosa, даже если в файле в колонке «вид» написано что-то другое.

Учитель может подставить во вторую мини-студию **готовый облачный проект** или **снимок шаблона** — тогда часть цепочки уже будет в окне; иначе собери её сам по смыслу выше.`
      },
      {
        id: "q09b_studio",
        type: "studio",
        instruction:
          "Во втором окне: снова **обучись** на Iris, затем **прогнай предсказание** по **всему** лабораторному файлу и отфильтруй строки, где предсказанный класс — **Iris-setosa**, с выводом на сцену.",
        stageInstruction:
          "**Сцена:** **Старт** → график обучения; после предсказания — **список ответов** внизу. Ветка «если» — про **предсказанный класс**.",
        studioPracticeKind: "empty",
        studioWorkspaceLevel: 1,
        goals: [
          { id: "q9b_goal_dataset", title: "Табличные данные для шага готовы", type: "select_dataset", datasetKind: "tabular" },
          { id: "q9b_goal_train_block", title: "В цепочке есть обучение", type: "add_block", blockType: "noda_train_model_simple" },
          { id: "q9b_goal_predict_block", title: "В цепочке есть предсказание", type: "add_block", blockType: "noda_predict_l1" },
          { id: "q9b_goal_if", title: "В цепочке есть ветвление «если … то»", type: "add_block", blockType: "noda_if_then_only" },
          { id: "q9b_goal_train", title: "Запусти обучение", type: "train_model" },
          { id: "q9b_goal_predict", title: "Сделай предсказание по лабораторному файлу", type: "run_prediction" }
        ]
      },
      {
...
- **Мини-студия** — уроковое окно Nodly; **полная разработка** в меню — для своих больших проектов.  
- На **Iris** учим честную связь «числа → вид»; лабораторный файл — для **предсказаний** и отбора **setosa** по спецификации; не смешивай роли файлов в одном шаге.  
- Два окна подряд — учебный приём: второе **не подхватывает** модель из первого; в реальной жизни модель из библиотеки можно подключить без переобучения — здесь это **специально упростили**.  
- Вторую мини-студию можно настроить в админке (**пусто / снимок шаблона / копия облачного проекта**).`
      },
...
Мы отделили **настоящую Iris setosa** от обмана в колонке «вид» — партию снова можно **проверять по правилам**, опираясь на цепочку **данные → обучение → проверка → предсказание**.

- ИИ учится на данных, не «магия».
- Нужна честная разметка для учёбы и **честная** оценка на отложенных примерах.
- **Метрики** и **Nodly** помогают доверять не одной цифре «уверенности», а поведению модели на train/test и на новом файле.`
      },
      { id: "q_div_10", type: "divider" },
      {
        id: "q14_itog_chain",
        type: "checkpoint",
        question: "Что из этого — разумная опора, когда в лабораторном файле метки подозрительны?",
        answerMode: "multi",
        options: [
          "Обучить модель на честном Iris с правильными метками",
          "Смотреть на test accuracy и динамику loss, а не только на «уверенность»",
          "Использовать предсказание по признакам и сравнивать с нужным видом в условии",
          "Полностью доверять колонке «вид» в лабораторном файле"
        ],
        expectedAnswer: "Обучить модель на честном Iris с правильными метками||Смотреть на test accuracy и динамику loss, а не только на «уверенность»||Использовать предсказание по признакам и сравнивать с нужным видом в условии"
      },
      {
        id: "q15_itog_two_windows",
        type: "checkpoint",
        question: "Почему во второй мини-студии цепочка начинается «с нуля», без модели из первого окна?",
        answerMode: "single",
        options: [
          "Это отдельный проект/сеанс: так в учебном сценарии показывают, что «перенос» надо делать осознанно",
          "Потому что Nodly не умеет сохранять модели",
          "Потому что Iris.csv нельзя открыть два раза"
        ],
        expectedAnswer: "Это отдельный проект/сеанс: так в учебном сценарии показывают, что «перенос» надо делать осознанно"
      },
      { id: "q_div_11", type: "divider" },
      {
        id: "q16_farewell",
        type: "text",
        body: `### До встречи

Это был **первый квест** серии — на сегодня всё, **увидимся в следующий раз**.

Во **втором занятии** подробно разберём **регрессию по таблице**: уже не «какой из классов», а **какое число** предсказываем по признакам — с нормальной постановкой задачи, метриками и ловушками данных. История будет про **моллюсков**, с датасетом, где важно связать измерения с **размером** (и снова проверять, не врёт ли подпись или измеритель).

Про «картинки вместо таблицы» в этом модуле **не обещаем** — сейчас весь фокус на **табличных** цепочках **данные → обучение → проверка → предсказание**.`
      }
    ],
    presentationPdfUrl: null,
    slides: [],
