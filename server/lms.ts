...
Ты в **цифровом Архиве Noda** — базе, где лежат данные про истории, растения, голоса, картинки и модели. Тебя встречает **Нодус**: он следит за порядком и по ходу объяснит, что к чему.

**Стажёр, у нас первый вызов.** В оранжерее с редкими ирисами **сбились подписи** — система путает похожие цветы. По сюжету дела нужен **пигмент** одного вида, чтобы остановить «**Серебряную пыльцу**». Руками всё не перебрать: нужно быстро понять, как по **цифрам** отличать виды — через обучение модели в **Nodly**.`
      },
      {
        id: "q00_hero",
        type: "media",
        ...
        caption: "Зал Архива, дальше оранжерея — отсюда заводим дело."
      },
      {
        id: "q00_greenhouse",
        type: "media",
        ...
        caption: "Ирисы и таблица рядом. Подпись сбилась — ориентируйся на числа в колонках."
      },
      {
        id: "q00_story_lab",
        type: "text",
...
В зале — замеры и журнал. В оранжерее те же цветы: если в данных не видно **вида**, не понять, к какому растению относится пигмент и сюжет про **Серебряную пыльцу**.`
      },
      {
        id: "q00_problem",
        type: "text",
...
Нужна **классификация** по длинам и ширинам лепестков и чашелистиков: сначала данные и обучение, потом проверка и предсказание. Ниже разберём это на таблице **Iris** в **Nodly** — ответ можно сверить по цифрам.`
      },
...
Я сую тебе столбцы длин и ширин, без фотографии «как в жизни». Вопрос простой: **можно ли отличить объект, глядя только на числа**?
...
Слева в студии: **Обучение** — датасет для блока «Обучить модель»; **Входы** — строки для «Предсказать»; **Модели** — сохранённые модели. Отдельного блока «датасет» в Blockly нет.`
      },
      {
        id: "q03_datasets_io",
        type: "text",
...
1. Данные → 2. Разметка → 3. Train/test → 4. Обучение → 5. Проверка → 6. Новые примеры.`
      },
      {
        ...
        type: "media",
        kind: "image",
        url: "/api/coach/iris-quest-dataset-split.png?v=2",
...
Датасет для обучения и строки для предсказания задаются во вкладке **«Данные»**, не отдельным блоком.
...
Допуск к студии: собери цепочку **Iris → обучение → предсказание** и проверь, держит ли модель смысл.`
      },
      {
        ...
        kind: "image",
        url: "/api/coach/iris-quest-hero.png",
        caption: "Зал Архива перед запуском студии: тот же узел, откуда заводили расследование."
      },
      {
        id: "q09_studio",
        type: "studio",
      ...
      },
      {
        ...
        type: "text",
        body: `### После первой практики

Во вкладке **«Данные»** можно загрузить свой CSV и при необходимости указать **целевую колонку**. Если таблиц в проекте нет, подставляется **Iris.csv** — выбери его в **«Обучение»**. Несколько строк для проверки — в **«Входы»** или вручную в блоке предсказания.

Часто при коротком лепестке модель даёт **Iris-setosa**. Дальше — **вторая разработка** в Nodly: нужна цепочка «если класс совпал с Iris-setosa — показать сообщение».`
      },
      { id: "q_div_09a", type: "divider" },
      {
        id: "q09b_setosa_intro",
        type: "text",
        textScale: "lg",
        body: `## Сцена 10. Вторая разработка

Откроется **новый** проект в Nodly (первый не трогай). Снова: Iris → обучить → предсказать. После предсказания: блок **если … то**, в условии сравни **предсказанный класс** с текстом **Iris-setosa**, в ветке «да» — **показать сообщение**. В **«Входы»** можно внести несколько строк и прогнать по очереди.`
      },
      {
        id: "q09b_studio_setosa",
        type: "studio",
        instruction:
          "Новый проект: Iris, обучение, предсказание, потом если предсказанный класс = Iris-setosa — показать сообщение. Входы по желанию.",
        studioPracticeKind: "empty",
        studioWorkspaceLevel: 1,
        goals: [
          { id: "q9b_dataset", title: "Выбери tabular-датасет Iris", type: "select_dataset", datasetKind: "tabular" },
          { id: "q9b_train_block", title: "Добавь блок «Обучить модель»", type: "add_block", blockType: "noda_train_model_simple" },
          { id: "q9b_train", title: "Запусти обучение модели", type: "train_model" },
          { id: "q9b_predict", title: "Сделай предсказание", type: "run_prediction" },
          { id: "q9b_if", title: "Добавь блок «если … то»", type: "add_block", blockType: "noda_if_then" },
          { id: "q9b_pred_class", title: "Добавь значение «предсказанный класс»", type: "add_block", blockType: "noda_value_predicted_class" },
          { id: "q9b_msg", title: "Добавь «показать сообщение»", type: "add_block", blockType: "noda_show_message" }
        ]
      },
...
**Дело 002: изображения** — модель учится по картинкам, не по столбцам таблицы.`
  ...
  await prisma.lessonTemplate.upsert({
    ...
    create: {
      ...
      description: "Детективное дело: основы ИИ, Iris dataset и первая модель в Nodly.",
      moduleKey: "module_a",
      sortOrder: 0,
      starterPayload: EMPTY_SNAPSHOT,
      ...
      studentSummary: "Архив Noda, Нодус, Iris в Nodly: две короткие практики."
    },
    update: {
      title: "Дело Ирисового шифра",
      description: "Детективное дело: основы ИИ, Iris dataset и первая модель в Nodly.",
