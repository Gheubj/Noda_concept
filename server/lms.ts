...
const LESSON_GUIDE_SEED: Record<string, { teacherGuideMd: string; studentSummary: string }> = {
  lt_intro_ai: {
    studentSummary:
      "Квест с Хранителем историй: как ИИ учится на данных, где он помогает в жизни и как собрать свою первую мини-модель в Noda.",
          ...
          elements: [
            {
              ...
              zIndex: 1,
              block: { id: "a_minidev_intro", type: "text", body: "## Мини-разработка: Первый запуск ИИ в Noda\nСобери цепочку из блоков и запусти мини-эксперимент.\n\nЦель: пройти путь **данные -> обучение -> предсказание**." }
            },
            {
              id: "a_e_19",
...
Ты получаешь временной доступ в **цифровой Архив Noda** — хранилище, где «лежат» не только файлы, но и истории: как учились модели, какие датасеты спасали проекты, где люди ошибались и что потом исправляли. Сегодня ты не зритель: ты оформляешься как **стажер-детектив данных**, и твой первый кейс начинается сразу, без длинного вступления в аудитории.

Твой наставник — **Нодус**. Он говорит спокойно, без лозунгов: короткие фразы, много пауз, много внимания к деталям. Он не просит «поверить в ИИ». Он просит **понять цепочку**: что за наблюдения стоят за ответом, где слабое место расследования, почему одной уверенности мало.

Обычно первый день в Академии похож на экскурсию: интерфейс, кнопки, правила. Но Архив не умеет ждать. Внутри учебной симуляции загорается **инцидент**: в оранжерее пропадают подписи к редким ирисам, а в логах всплывает вымышленная **Серебряная пыльца** — сценарий, который учит думать о данных всерьёз. По легенде Архива, остановить распространение может пигмент только одного вида ириса. Если мы ошибёмся с классом, мы потеряем время — а в симуляции время для стажера стоит дорого.

Нодус смотрит на тебя и произносит ровно: «Сначала ты поймёшь, что такое ИИ и где он уже живёт рядом с людьми. Потом — что такое данные и почему без разметки модель не знает, чему учиться. И только потом вернёмся к ирисам: там будет таблица, цифры и твоя первая модель в Noda. Без паники. По шагам.»

Твоя роль проста: **не пропускать смысл**. Если что-то звучит как «магия», остановись и переспроси себя: какие наблюдения за этим стоят?`
      },
      {
        id: "q00_hero",
        type: "media",
        ...
        caption: "Лаборатория Архива: Нодус проводит тебя к месту инцидента."
      },
      {
        id: "q00_story_lab",
        type: "text",
        body: `### Почему это похоже на лабораторию

В Архиве нет «красивых картинок ради картинок». Есть стол, на котором лежат измерения, посуда с оттенками, экраны с графиками и тихий гул вентиляции. Здесь учатся относиться к цифрам как к **уликам**: аккуратно, без спешки, с проверкой.

Нодус любит повторять: «Каждая модель оставляет следы в данных». Это значит, что если мы честно соберём цепочку «пример → ответ → проверка», то даже простая модель может выручить лучше, чем красивый рассказ без фактов.

Ниже ты увидишь оранжерею — визуально это другая зона Архива, но логика та же: где-то сломалась разметка, и без восстановления классов мы не узнаем, какой вид ириса нужен для «противоядия» в учебной истории.`
      },
      {
        id: "q00_problem",
        type: "text",
        body: `## Экстренный вызов Нодуса

Инцидент в отчёте выглядит сухо, но для оранжереи это настоящая авария: **часть подписей к редким ирисам повреждена**, а автоматические проверки начинают путать похожие образцы. Вручную перебрать сотни растений можно, но это медленно — и именно в спешке чаще всего появляются новые ошибки в журнале.

Параллельно симуляция поднимает «тревогу» из-за **Серебряной пыльцы** — вымышленного сюжетного мотиватора, который отражает реальную мысль курса: **если не восстановить классификацию**, система не сможет принять правильное решение о редком ресурсе. По легенде Архива, остановить распространение может пигмент только одного вида ириса.

Нодус не предлагает «угадать цветок на глаз». Он предлагает честный инженерный путь: **собрать модель**, которая по измерениям лепестков и чашелистиков восстанавливает вид. Это не волшебная кнопка. Это цепочка: данные → обучение → проверка → предсказание.

**Задача стажера:** пройти брифинг по ИИ и данным, затем в Noda собрать первую модель на Iris и убедиться, что ответы модели можно **проверить**, а не только «красиво посмотреть».`
      },
      {
        id: "q00_greenhouse",
        type: "media",
        ...
        caption: "Архивная оранжерея: панели с подписями «ломаются», как стекло — так Архив показывает, что данные бывают хрупкими."
      },
...
Нодус кладёт перед тобой распечатку: ровные столбцы чисел — длины и ширины, без фотографии «как в жизни». Он не торопит с ответом. Он хочет, чтобы ты сам сформулировал мысль: **можно ли вообще отличать живой объект, если смотреть только на числа**, которые описывают его форму.

Если ты согласишься, что числа могут нести смысл, дальше станет понятно, зачем в Архиве завели таблицу Iris и почему от неё зависит «пигмент» в истории про Серебряную пыльцу. Если нет — Нодус предложит другую аналогию, но тогда путь к модели будет длиннее.`
      },
      {
        id: "q00_hypothesis",
        type: "checkpoint",
...
Нодус переводит разговор в сторону привычного мира — не ради «интересностей», а чтобы ты почувствовал масштаб: **ИИ уже встроен в сервисы**, которыми ты пользуешься. Это не отдельная магия, а часть инженерии: сбор сигналов, поиск повторяемости, проверка качества.

ИИ — это не «человеческое сознание в коробке». Это программы, которые учатся на примерах, ищут закономерности в данных и делают предсказания. Иногда предсказание выглядит как рекомендация видео, иногда — как подсказка по снимку, а в нашем учебном случае — как **классификация вида цветка** по измерениям.

Дальше Нодус предложит тебе маленькую «стену улик»: сервисы вокруг, где ИИ делает заметную работу. Запомни главное: если ИИ умеет удерживать закономерность в шумных данных, то и с ирисами у него есть шанс — при условии честной разметки и проверки.`
      },
      {
        id: "q01_ai_examples",
        type: "text",
...
- **YouTube / TikTok / Netflix**: анализируют, что ты смотришь, лайкаешь и пропускаешь.
- **Фильтры лица**: находят точки лица и двигают маску вместе с тобой.
- **Голосовые помощники**: превращают звук в слова и понимают намерение.
- **Игры**: NPC реагируют, противники выбирают действия, сложность адаптируется.
- **Переводчики**: распознают текст и подбирают смысл на другом языке.

Если ИИ может узнать лицо, голос и вкус зрителя, значит он может помочь узнать и вид ириса.`
      },
      {
        id: "q01_ai_or_not",
        type: "checkpoint",
...

      {
        id: "q02_what_is_ai",
        type: "text",
        body: `## Сцена 2. Что такое ИИ на самом деле

**Искусственный интеллект** — это программа, которая:

- учится на примерах;
- ищет закономерности в данных;
- делает предсказание или решение;
- улучшается через практику и проверку.

Главный секрет: ИИ не думает как человек. Он находит **математические связи**.`
      },
      {
        id: "q02_models_tasks",
        type: "text",
        body: `### Базовые задачи ИИ

| Задача | Что делает | Пример |
|---|---|---|
| Классификация | выбирает класс | вид ириса, спам/не спам |
| Регрессия | предсказывает число | цена, температура, оценка |
| Распознавание | находит объект/речь | лицо, голос, цифра |
| Генерация | создает новое | текст, картинка, музыка |
| Рекомендации | подбирает вариант | видео, музыка, товары |

Для дела Ирисов нам нужна **классификация**: выбрать один из трех видов.`
      },
      {
        id: "q02_model_types",
        type: "text",
        body: `### Базовые модели простыми словами

- **Правила**: если условие выполнено — делай действие.
- **Дерево решений**: серия вопросов как детективная схема.
- **k ближайших соседей**: ищет похожие примеры рядом.
- **Нейросеть**: много внутренних настроек, которые учатся на ошибках.

Сегодня важнее понять цикл: **данные → обучение → проверка → предсказание**.`
      },
      {
        id: "q02_check",
        type: "checkpoint",
...

      {
        id: "q03_data_title",
        type: "text",
        body: `## Сцена 3. Данные — еда для ИИ

Нодус открывает хранилище улик. В нем не заклинания, а данные.
...
Без данных модель не учится. Без качественных данных модель учится плохо.`
      },
      {
        id: "q03_labels",
        type: "text",
...
Разметка — это подписи к примерам.

Примеры:
- фото: «кошка» или «собака»;
- письмо: «спам» или «не спам»;
- цветок: «setosa», «versicolor» или «virginica».

Если подписи повреждены, модель получает плохие уроки. Поэтому качество разметки — часть расследования.`
      },
      {
        id: "q03_data_check",
        type: "checkpoint",
...
Нодус достает из Архива первое досье. В оранжерее три вида:

- **Iris setosa**
- **Iris versicolor**
- **Iris virginica**

Один вид содержит нужный пигмент против Серебряной пыльцы. Но подписи у части цветов исчезли. Остались только измерения.`
      },
      {
        ...
        caption: "Три вида ирисов на панели анализа Архива: система сравнивает признаки и классы."
      },
      {
        id: "q04_table",
        type: "text",
...
- задача = классификация.`
...

      {
        ...
        type: "text",
        body: `## Сцена 5. Как модель учится

Расследование превращается в конвейер:
...
4. Обучение.
5. Проверка.
6. Использование на новых данных.`
      },
      {
        id: "q05_split_image",
        type: "media",
...
- **Train** — учебник модели. На этих примерах она учится.
- **Test** — контрольная. Эти примеры модель не должна видеть заранее.

Часто начинают с принципа 80/20: 80% данных на обучение, 20% на проверку.

Если показать модели тест заранее, она может просто запомнить ответы.`
      },
      {
        id: "q05_split_check",
        type: "checkpoint",
...

      {
        ...
        type: "text",
        body: `## Сцена 6. Метрики: можно ли доверять модели

Нодус предупреждает: уверенный ответ не всегда правильный.

Метрики нужны, чтобы не верить модели на слово.`
      },
      {
        id: "q06_metrics_text",
        type: "text",
        body: `### Главные показатели

- **Accuracy** — доля правильных ответов.
- **Confidence** — насколько модель уверена в конкретном ответе.
- **Loss** — насколько сильно модель ошибается при обучении.

Loss должен уменьшаться. Accuracy на тестовой выборке важнее красивой уверенности в одном ответе.`
      },
      {
        id: "q06_trust_check",
        type: "checkpoint",
...

      {
        id: "q07_fit_title",
        type: "text",
        body: `## Сцена 7. Ловушки обучения

Есть два частых провала:

### Недообучение
Модель слишком простая и не поняла закономерности.

### Переобучение
Модель выучила тренировочные примеры, но плохо работает на новых.`
      },
      {
        id: "q07_fit_image",
        type: "media",
...
- Недообучение: ученик почти не готовился и не понял тему.
- Переобучение: ученик заучил ответы из одного варианта, но провалился на другой контрольной.

Диагноз по метрикам:
- train низкий, test низкий → недообучение;
- train высокий, test низкий → переобучение.`
      },
      {
        id: "q07_fit_check",
        type: "checkpoint",
...

      {
        id: "q08_noda_title",
        type: "text",
        body: `## Сцена 8. Noda: набор инструментов стажера

Нодус выдает инструменты расследования: блоки Noda.

Здесь не нужно писать код с нуля. Логику можно собрать как цепочку действий.`
      },
      {
        id: "q08_blocks_image",
        type: "media",
        ...
        caption: "Блоки Noda: события, данные, обучение, предсказание, управление и действия."
      },
      {
        id: "q08_blocks_text",
        type: "text",
...
- **Старт** — запускает программу.
- **Данные** — число или текст.
- **Обучить модель** — модель учится по датасету.
- **Предсказать** — модель дает ответ.
- **Уверенность** — насколько модель уверена.
- **Предсказанный класс** — итог классификации.
- **Показать результат / журнал** — выводит ответ пользователю.`
      },
      {
        id: "q08_blocks_check",
        type: "checkpoint",
...

      {
        ...
        type: "text",
        textScale: "lg",
        body: `## Сцена 9. Финальная практика: спасти оранжерею

Теперь стажер допускается к аварийной лаборатории.

Нужно собрать мини-разработку на Iris и проверить, сможет ли модель восстановить подписи цветов.`
      },
      {
        id: "q09_lab_image",
        type: "media",
        kind: "image",
        url: "/api/coach/iris-quest-lab.png",
        caption: "Аварийная лаборатория Архива: здесь собирают цепочку «данные → модель → предсказание»."
      },
      {
        id: "q09_studio",
        type: "studio",
        instruction:
          "Миссия стажера: выбери tabular-датасет Iris, добавь блок обучения, запусти обучение и сделай предсказание для нового цветка из аварийной оранжереи. Посмотри на уверенность и подумай: можно ли доверять ответу модели?",
        studioPracticeKind: "empty",
        studioWorkspaceLevel: 1,
        goals: [
...
Нодус фиксирует результат:

- ИИ не магия.
- ИИ учится на данных.
- Данные должны быть размечены.
- Модель нужно проверять.
- Метрики помогают доверять результату.
- Noda позволяет собрать первый ИИ без кода.`
      },
      {
        id: "q10_final_image",
        type: "media",
        kind: "image",
        url: "/api/coach/success.png",
        caption: "Нодус: первый день стажировки завершен. Дело Ирисового шифра закрыто."
      },
      {
        id: "q10_final_hook",
        type: "text",
...
Нодус открывает новую карточку дела:

**Дело 002: изображения.**

В следующем расследовании модель будет учиться распознавать объекты не по таблицам, а по картинкам.`
      },
      {
        id: "q10_final_check",
        type: "checkpoint",
