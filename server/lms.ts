...
- **Метрики** и **Nodly** помогают доверять не одной цифре «уверенности», а поведению модели на train/test и на новом файле.`
      ...
      { id: "q_div_10", type: "divider" },
      {
        ...
        caption: "Архив: дело с таблицей закрыто, можно переходить к следующей теме."
      },
      {
        id: "q12_next_folder",
        type: "text",
...
**Дело 002: изображения** — там модель учится по **картинкам**, а не по столбцам таблицы. Истории с **битыми подписями** и **чужим вмешательством** всплывают и там — привыкай проверять ту же цепочку **данные → обучение → проверка → предсказание**.`
      },
...
Ты прошёл **табличное** дело модуля A: дальше — **изображения**, другие входы и другие ловушки данных — с теми же привычками проверки цепочки.`
      }
    ],
    presentationPdfUrl: null,
    slides: [],
