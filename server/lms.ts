...
Потом коротко про ИИ и данные; в конце — две **мини-разработки** в Nodly: **обучить и сохранить модель**, затем **прогнать «битый» лабораторный список** и отметить строки, где программа уверенно видит **setosa**.`
      },
      {
        id: "q00_hero",
        type: "media",
...
Слово **«классификация»** дальше будет значить ровно это: **по числам выбрать один из трёх видов**. Файлы **Iris.csv** и **Оранжерея лаборатории** подключишь в Nodly в разделе **«Данные»** — к блокам в цепочке вернёмся, когда дойдём до мини-разработки.`
      },
...
Инструменты стажера — **блоки**: собираешь цепочку **обучить → предсказать → если setosa — показать**, без кода с нуля. Так быстрее закрыть **срочную** партию для единорогов.`
      },
      {
        id: "q08_blocks_image",
        type: "media",
...
**Iris (Iris.csv)** — честные примеры, на них **учимся**. **Оранжерея лаборатории** — выгрузка после оранжереи: по ней **предсказываем** и ищем **настоящую setosa** для корма (одна строка или **весь файл**; список — на вкладке «Визуализация» и на сцене мини-студии).`
      },
      {
        id: "q09_lab_title",
        type: "text",
...
**Первая вкладка** — обучить классификатор на **Iris** и **сохранить модель** в библиотеку. **Вторая вкладка** — новый черновок среды: снова коротко **обучись на Iris**, затем **Предсказать** по лабораторному файлу (лучше **весь файл**), **если … то** и сообщение/результат для **Iris-setosa** — это строки, которые **можно класть в корм**, пока колонка «вид» в файле **ненадёжна**.
...
На сцене мини-студии — короткие подсказки и график обучения; длинный текст — только в ленте урока слева.`
      },
      {
        ...
        kind: "image",
        url: "/api/coach/iris-quest-greenhouse.png",
        caption: "Оранжерея и лабораторный файл; Нодус — дальше две вкладки мини-студии по очереди."
      },
...
Открылась **вторая** мини-студия (новый облачный черновик): модель из шага 1 здесь не переносится — как после переустановки. **Снова** обучись на **Iris**, затем **Предсказать** по **Оранжерея лаборатории** (**весь файл**), добавь **если … то (без иначе)** и **показать сообщение** или **показать результат** для **Iris-setosa** — именно эти строки идут в **корм**, пока метки в файле после **чужого вмешательства** ненадёжны. Список ответов — внизу сцены после графика.`
      },
      {
        id: "q09b_studio",
        type: "studio",
        instruction:
          "**Старт** → **Обучить модель** (Iris) → **Предсказать** (лаборатория, **весь файл**) → **если** предсказанный класс = **Iris-setosa** → **показать сообщение** / **результат** (в корм). Смотри график при обучении.",
        studioPracticeKind: "empty",
        studioWorkspaceLevel: 1,
        goals: [
...
Учебный и лабораторный файлы не смешивать: на **Iris** учим **честную** связь чисел и вида, лабораторный — только для **предсказаний** и отбора **setosa** в корм. В «боевом» проекте шаг 1 оставил бы модель в библиотеке для шага 2 без переобучения — в квесте второй черновик это имитирует отдельную сессию.`
      },
