...
Ты стажёр в **Агентстве историй Нодус**: мы оформляем понятный отчёт — **как вытащили** ситуацию, когда в таблице не всё честно, а времени почти нет.

**Нодус** ведёт это дело.

Единороги едят корм, в который по рецепту входят **только лепестки Iris setosa**. Без этой партии они **останутся голодными** — отгрузка нужна **срочно**.

В оранжерее **кто-то** **разбил часть табличек** над цветами, а в выгрузке **испортилась колонка «вид»**: где-то пусто, где-то указан не тот вид. Сами **числа** (длины и ширины чашелистика и лепестка) измерены как обычно — сломалась именно **подпись вида** в таблице.

Руками сотни строк не проверить: нужна **модель в Nodly**, которая по четырём числам находит **настоящую setosa** — ровно то, что нужно в корм.

Дальше — коротко про ИИ, данные и Iris; потом две мини-разработки: **обучить и сохранить**, затем **прогнать лабораторный файл и отобрать setosa условием в цепочке**.`
      },
      {
        id: "q00_hero",
        type: "media",
      ...
      },
      {
        ...
        caption: "Оранжерея: три вида ирисов; после того как кто-то поломал надписи, в файле колонка «вид» ненадёжна."
      },
      {
        id: "q00_story_lab",
        type: "text",
...
В зале — регламент и отчёт. В оранжереи — **цветы**. Для корма нужна строго **Iris setosa**. **Кто-то** оставил след в **подписях** — поэтому дальше работаем по **числам** и модели, а не по испорченной колонке «вид».`
      },
      {
        id: "q00_problem",
        type: "text",
...
Нужна **классификация** по четырём измерениям листа и лепестка: сначала обучение на честном **Iris.csv**, потом проверка на **лабораторном файле** из оранжереи и отбор строк, где реальная **setosa** — именно она идёт в **корм**. В **Nodly** ты соберёшь цепочку, по которой можно **срочно** доказать, что партию для единорогов можно собрать по правилам.`
      },
...
В деле только **таблица**: четыре числа на цветок, без фотографий — как в учебной задаче. Вопрос: **можно ли по числам понять вид** — и тем самым отделить **настоящую setosa** от строк, где в колонке «вид» после **чужого вмешательства** оказалась **неправда**?`
      },
      {
        id: "q00_hypothesis",
        type: "checkpoint",
...
**ИИ уже в сервисах**, которыми ты пользуешься: не «магия», а программы, которые учатся на примерах и ищут закономерности в данных.

В нашем деле это **классификация вида цветка** по числам — тот же приём, что помогает не перепутать **корм для единорогов**, когда подписи в файле ненадёжны.`
      },
      {
        id: "q01_ai_examples",
        type: "text",
