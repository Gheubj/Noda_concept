...
Сегодня ты не в аудитории — ты в **цифровом Архиве Noda**. Здесь хранятся не только файлы, но и истории: какие датасеты спасали проекты, где люди ошибались и что потом чинили. Ты оформляешься как **стажер-детектив данных**, и первый кейс начинается сразу.

Твой наставник — **Нодус**. Он говорит спокойно, без лозунгов: коротко, с паузами, с любовью к деталям. Он не просит «поверить в ИИ». Он просит **собрать цепочку**: что за наблюдения стоят за ответом, где слабое место расследования, почему одной уверенности мало.

Внутри учебной симуляции загорается **инцидент**: в оранжерее пропадают подписи к редким ирисам, а в логах всплывает вымышленная **Серебряная пыльца** — сценарий, который учит думать о данных всерьёз. По легенде Архива, остановить распространение может пигмент только одного вида ириса. Ошибись с классом — потеряешь время, а в симуляции время для стажера стоит дорого.
...
Твоя роль проста: **не пропускать смысл**. Если что-то звучит как «магия», остановись и переспроси себя: какие наблюдения за этим стоят?`
      },
      {
        ...
        caption: "Главный лабораторный зал Архива: стекло, потоки данных на столе и оранжерея на фоне. Отсюда Нодус заводит дело."
      },
      {
        id: "q00_story_lab",
        type: "text",
...
Так в Архиве называют этот зал: не «декорация», а рабочее место. Здесь сравнивают измерения, смотрят на графики и ведут журнал так, чтобы потом можно было **восстановить ход расследования**. Нодус любит повторять: «Каждая модель оставляет следы в данных».

Ниже — оранжерея: другая зона, но логика та же. Где-то сломалась разметка, и без восстановления классов мы не узнаем, какой вид ириса нужен для «противоядия» в учебной истории.`
      },
      {
        id: "q00_problem",
        type: "text",
...
В отчёте это звучит сухо, но для оранжереи это авария: **часть подписей к редким ирисам повреждена**, а автоматические проверки начинают путать похожие образцы. Вручную перебрать сотни растений можно, но медленно — и в спешке чаще всего появляются новые ошибки в журнале.

Параллельно симуляция поднимает «тревогу» из-за **Серебряной пыльцы** — вымышленного мотиватора, который отражает реальную мысль курса: **если не восстановить классификацию**, система не сможет принять правильное решение о редком ресурсе.

Нодус не предлагает «угадать цветок на глаз». Он предлагает честный путь: **собрать модель**, которая по измерениям лепестков и чашелистиков восстанавливает вид. Это цепочка: данные → обучение → проверка → предсказание.
...
Если да — дальше станет понятно, зачем в Архиве завели таблицу Iris и почему от неё зависит «пигмент» в истории про Серебряную пыльцу. Если нет — Нодус предложит другую аналогию, но путь к модели станет длиннее.`
      },
      {
        id: "q00_hypothesis",
        type: "checkpoint",
...
ИИ — это не «человеческое сознание в коробке». Это программы, которые учатся на примерах, ищут закономерности в данных и делают предсказания. Иногда это рекомендация видео, иногда подсказка по снимку, а в нашем учебном случае — **классификация вида цветка** по измерениям.

Дальше — маленькая «стена улик»: сервисы вокруг, где ИИ делает заметную работу. Запомни главное: если ИИ умеет удерживать закономерность в шумных данных, то и с ирисами у него есть шанс — при честной разметке и проверке.`
      },
      {
        id: "q01_ai_examples",
        type: "text",
