...
Ты в **цифровом Архиве Nodly** — не как зритель, а как **стажер-детектив данных**. Я веду без лозунгов: коротко, по фактам, с вопросом «какая цепочка наблюдений за этим стоит?».

В симуляции загорается **инцидент**: в оранжерее ломаются подписи к редким ирисам, в логах — **Серебряная пыльца**. По легенде Архива спасти ситуацию может пигмент только одного вида.
...
Твоя задача — **не пропускать смысл**: если звучит как магия, найди, какие данные за этим спрятаны.`
      },
      {
        ...
        caption: "Главный зал Архива: стол с потоками данных, в глубине — оранжерея. Здесь мы заводим дело."
      },
      {
        id: "q00_greenhouse",
        type: "media",
        ...
        caption: "Оранжерея: ирисы под панелями данных. Трещины — намёк, что разметка может быть хрупкой."
      },
      {
        id: "q00_story_lab",
        type: "text",
...
Зал — рабочее место: измерения, графики, журнал, по которому потом **восстанавливают ход расследования**.
...
Путь: **модель по измерениям лепестков и чашелистиков** — не угадывание, а цепочка: данные → обучение → проверка → предсказание.

**Стажер:** брифинг, затем в Nodly — Iris и ответ, который можно **проверить**.`
      },
...
Форматы: изображения, звук, текст, таблицы чисел.`
      },
      {
        id: "q03_datasets_io",
        type: "text",
        body: `### Датасеты, загрузка и входы в Nodly

**Датасет в проекте** — это таблица или набор примеров, который ты выбираешь для обучения (например Iris). Модель читает строки: столбцы с **признаками** и столбец с **меткой**, пока ты не сменишь источник.

**Загрузка** — подключение готового учебного набора или файла (часто CSV), если среда это позволяет. Важно: имена и типы столбцов должны совпадать с тем, что ждут блоки обучения и предсказания.

**Входы при предсказании** — это **новые** значения признаков (для Iris четыре числа), которые ты подаёшь в блок предсказания *после* обучения. Это не «ещё строки из train», а отдельный пример: «цветок из оранжереи сейчас».

Итого: **датасет = история и уроки**, **вход при предсказании = сегодняшняя улика** — их нельзя путать.`
      },
      {
...
Инструменты стажера — **блоки**: логику собирают цепочкой, без кода с нуля.`
      ...
      },
      {
        id: "q08_blocks_text",
        type: "text",
        body: `### Что делает каждый блок

- **При запуске / Старт** — запускает программу и цепочку блоков, подключённых к событию старта.
- **Данные (число или текст)** — подставляет значение в цепочку: признаки цветка, подпись, константу для проверки.
- **Выбрать датасет** — привязывает к проекту табличный или другой датасет; без этого модель не на чем учиться.
- **Обучить модель** — берёт выбранный датасет и запускает обучение классификатора; обновляет внутреннее состояние модели в проекте.
- **Предсказать** — прогоняет обученную модель на новых входных признаках и получает ответ (класс + внутренние сигналы).
- **Уверенность** — возвращает число уверенности для последнего предсказания: полезно, чтобы не путать «громкий» ответ с надёжным.
- **Предсказанный класс** — отдаёт строку метки (например вид ириса), которую можно показать или записать в журнал.
- **Показать сообщение / журнал** — выводит текст ученику: итог дела, промежуточную улику, предупреждение.
- **Ждать N секунд** — пауза между шагами, если нужно разнести запуск по времени (в учебных сценариях).

Минимальная цепочка для нашего дела в **Nodly**: **Старт → датасет Iris → обучить модель → предсказать → уверенность/класс → показать результат**.`
      },
      {
        id: "q08_blocks_check",
        type: "checkpoint",
...
**Дело 002** — уже про картинки, не таблицы.`
      },
      {
        ...
        caption: "Лабораторный стол Архива после дела: образцы и реагенты рядом — как финальный кадр расследования, когда шум стих."
      },
      {
        id: "q10_final_hook",
        type: "text",
