    ...
    blocks: [
      {
        ...
        type: "text",
        textScale: "lg",
        body: `# Дело Хранителя: Архив Ирисов

Сегодня класс получает доступ к закрытому Архиву историй ИИ.

Но в Архиве сбой: древняя оранжерея Ирисов потеряла подписи к цветам. Хранитель знает истории, но не может восстановить порядок без модели.

**Роль учеников:** стать агентами данных, понять основы ИИ и помочь Хранителю собрать первую модель на датасете Iris.`
      },
      {
        id: "q_story_1",
        type: "media",
        kind: "image",
        url: "/api/coach/talking.png",
        caption: "Хранитель: сначала разберемся, что такое ИИ. Потом откроем дело Ирисов."
      },
      {
        id: "q_story_2",
        type: "text",
        body: `## Завязка для урока

Хранитель показывает три закрытые печати:

1. **Печать данных** — откуда ИИ берет знания.
2. **Печать обучения** — как модель находит закономерности.
3. **Печать проверки** — как понять, можно ли доверять результату.

Учитель ведет брифинг, ученики выполняют квестовые проверки и финальную мини-разработку.`
      },
      { id: "q_div_0", type: "divider" },
      {
        id: "q_life_1",
        type: "text",
        body: `## Сцена 1. ИИ уже рядом

Скажи честно: ты когда-нибудь задумывался, как телефон понимает речь, YouTube угадывает видео, а персонажи в играх реагируют на тебя?

Это не магия. Это искусственный интеллект — программы, которые учатся на данных и находят закономерности.`
      },
      {
        id: "q_life_2",
        type: "text",
        body: `### Пример: рекомендации

YouTube, TikTok и Netflix смотрят:
- что ты досмотрел;
- что лайкнул;
- что пропустил;
- на какие темы возвращаешься.

ИИ ищет людей с похожими вкусами и предлагает то, что понравилось им.`
      },
      {
        id: "q_life_3",
        type: "text",
        body: `### Пример: фильтры и лицо

Когда маска в камере держится на лице, ИИ:
- находит точки лица: глаза, нос, рот, контур;
- отслеживает движение;
- накладывает эффект в реальном времени.

Похожая идея используется в Face ID, поиске людей на фото и системах безопасности.`
      },
      {
        id: "q_life_4",
        type: "text",
        body: `### Пример: голос и игры

Голосовой помощник проходит цепочку:
1. слышит звук;
2. превращает его в признаки;
3. распознает слова;
4. понимает намерение;
5. отвечает.

В играх ИИ управляет противниками, NPC, сложностью и иногда генерацией миров.`
      },
      { id: "q_div_1", type: "divider" },
      {
        id: "q_ai_1",
        type: "text",
        body: `## Сцена 2. Что такое ИИ на самом деле

**ИИ** — это программа, которая:
- учится на примерах;
- ищет закономерности в данных;
- делает предсказания или решения;
...
Главный секрет: ИИ не думает как человек. Он находит математические связи.`
      },
      {
        id: "q_ai_2",
        type: "text",
        body: `### Простая аналогия

Ты учишься отличать собак от кошек:

1. Смотришь много примеров с подписями.
2. Замечаешь признаки: уши, нос, форма, размер.
3. Видишь новое фото и угадываешь.

Модель делает похожее, только вместо интуиции использует числа и формулы.`
      },
      {
        id: "q_ai_3",
        type: "text",
        body: `### Основные задачи ИИ

| Задача | Что делает | Пример |
|---|---|---|
| Классификация | выбирает класс | вид цветка, спам/не спам |
...
- **Дерево решений**: серия вопросов как в игре «да/нет».
- **k ближайших соседей**: ищет похожие примеры рядом.
- **Нейросеть**: много маленьких настроек, которые учатся на ошибках.

В первом уроке важно понять не формулы, а цикл: данные → обучение → проверка → предсказание.`
      },
      {
        id: "q_ai_check",
        type: "checkpoint",
        question: "Теперь можно открыть первую печать. Что ИИ делает с данными?",
        answerMode: "single",
        options: ["Ищет закономерности и делает предсказания", "Просто хранит файлы", "Всегда отвечает случайно"],
        expectedAnswer: "Ищет закономерности и делает предсказания"
      },
      { id: "q_div_2", type: "divider" },
      {
        id: "q_data_1",
        type: "text",
        body: `## Сцена 3. Данные — еда для ИИ

Без данных ИИ не может учиться. Как человек не может стать сильнее без практики, модель не может стать умнее без примеров.
...
- фото: «кошка», «собака»;
- письмо: «спам», «не спам»;
- цветок Iris: «setosa», «versicolor», «virginica».

Без разметки модель не понимает, какой ответ считать правильным.`
      },
      {
        id: "q_data_3",
        type: "text",
        body: `### Факты для учителя

- В мире каждый день создается гигантский объем данных.
- Для хорошей модели нужны не только много примеров, но и качественные примеры.
- Плохая разметка учит модель плохому поведению.
- Несбалансированные классы могут сделать модель «предвзятой» к одному ответу.`
      },
      {
        id: "q_data_check",
        type: "checkpoint",
        question: "Какие условия помогают модели учиться лучше?",
        answerMode: "multi",
        options: ["Качественная разметка", "Разнообразные примеры", "Случайные подписи", "Баланс классов"],
        expectedAnswer: "Качественная разметка||Разнообразные примеры||Баланс классов"
      },
      { id: "q_div_3", type: "divider" },
      {
        id: "q_iris_1",
        type: "text",
        textScale: "lg",
        body: `## Сцена 4. Дело Ирисов

В Архиве есть оранжерея с тремя видами ирисов:
- **Iris setosa**
- **Iris versicolor**
- **Iris virginica**

После сбоя подписи части цветов исчезли. Остались только измерения лепестков и чашелистиков. Нужно научить модель восстанавливать вид цветка.`
      },
      {
        id: "q_iris_2",
        type: "text",
        body: `### Как выглядит Iris dataset
...
Каждая строка — пример. Числа — признаки. Вид — правильный ответ.`
      },
      {
        id: "q_iris_3",
        type: "text",
...
Модель может заметить:
- если лепесток очень короткий → часто Setosa;
- если лепесток средний → часто Versicolor;
- если лепесток длинный и широкий → часто Virginica.

Человек видит таблицу. Модель видит числовое пространство и границы между классами.`
      },
      {
        id: "q_iris_check",
        type: "checkpoint",
        question: "В Iris dataset что является признаком, а что является меткой?",
        answerMode: "single",
        options: ["Признаки — числа измерений, метка — вид цветка", "Признак — название цветка, метка — длина", "Все столбцы являются метками"],
        expectedAnswer: "Признаки — числа измерений, метка — вид цветка"
      },
      { id: "q_div_4", type: "divider" },
      {
        id: "q_train_1",
        type: "text",
...
6. Использовать на новых данных.`
      },
      {
        id: "q_train_2",
        type: "text",
...
- **Обучающая выборка** — примеры, на которых модель учится.
- **Тестовая выборка** — примеры, которые модель не видела; на них проверяем честно.

Частый стартовый вариант: 80% train и 20% test.

Если модель заранее видела тест, она может просто запомнить ответы.`
      },
      {
        id: "q_train_3",
        type: "text",
        body: `### Эпохи и параметры

Во время обучения модель много раз пробует улучшить свои настройки.

- **Параметры** — числа внутри модели.
- **Эпоха** — один проход обучения по данным.
- Обычно для учебных задач достаточно нескольких или десятков эпох.

Модель не устает, но может переучиться — поэтому проверка обязательна.`
      },
      {
        id: "q_train_check",
        type: "checkpoint",
        question: "Зачем нужна тестовая выборка?",
        answerMode: "single",
        options: ["Чтобы честно проверить модель на новых примерах", "Чтобы удалить плохие строки", "Чтобы ускорить интернет"],
        expectedAnswer: "Чтобы честно проверить модель на новых примерах"
      },
      { id: "q_div_5", type: "divider" },
      {
        id: "q_metrics_1",
        type: "text",
        body: `## Сцена 6. Метрики: можно ли доверять модели

Когда модель дала ответ, важно спросить: «Насколько она хороша?»

Основные понятия:
- **Accuracy** — доля правильных ответов.
- **Confidence** — насколько модель уверена в конкретном ответе.
- **Loss** — насколько сильно модель ошибается во время обучения.`
      },
      {
        id: "q_metrics_2",
        type: "text",
        body: `### Loss простыми словами

Loss — число ошибки.

- Loss = 100 → модель ошибается сильно.
- Loss = 10 → ошибается меньше.
- Loss = 0.1 → почти попадает.

Обучение старается уменьшить Loss. Это похоже на спуск с горы: делаем шаг, проверяем, стало ли ниже, и продолжаем.`
      },
      {
        id: "q_metrics_3",
        type: "text",
        body: `### Пример прогресса

| Попытка | Что происходит | Accuracy | Loss |
|---:|---|---:|---:|
| 1 | угадывает почти случайно | 33% | высокий |
| 10 | нашла простые закономерности | 70% | средний |
| 100 | границы стали точнее | 95% | низкий |

Важно: хорошая модель должна быть хорошей не только на train, но и на test.`
      },
      {
        id: "q_metrics_check",
        type: "checkpoint",
        ...
        answerMode: "single",
        options: ["Насколько модель ошибается", "Сколько цветов в датасете", "Сколько блоков на экране"],
        expectedAnswer: "Насколько модель ошибается"
      },
      { id: "q_div_6", type: "divider" },
      {
        id: "q_fit_1",
        type: "text",
        body: `## Сцена 7. Две ловушки обучения

### Недообучение
Модель слишком простая и не поняла закономерности.

Признак: качество низкое и на train, и на test.

### Переобучение
Модель выучила тренировочные примеры, но плохо работает на новых.

Признак: train высокий, test заметно ниже.`
      },
      {
        id: "q_fit_2",
        type: "text",
        body: `### Аналогия для класса
...
Решения: больше качественных данных, баланс классов, правильная сложность модели, честный тест.`
      },
      {
        id: "q_fit_check",
        type: "checkpoint",
      ...
      { id: "q_div_7", type: "divider" },
      {
        id: "q_noda_1",
        type: "text",
        body: `## Сцена 8. Noda как лаборатория агента

Теперь теория превращается в действие.

В Noda ученик не пишет код с нуля, а собирает логику блоками:
- **Старт** запускает программу.
- **Данные** дают вход.
- **Обучить модель** запускает обучение.
- **Предсказать** получает ответ.
- **Уверенность** показывает, насколько модель уверена.
- **Показать результат / сообщение / журнал** выводит итог.`
      },
      {
        id: "q_noda_2",
        type: "text",
...
CSV — стандартный формат таблиц для данных.
...
- запятые разделяют столбцы;
- один столбец обычно содержит правильный класс.`
      },
      {
        id: "q_noda_3",
        type: "text",
        body: `### Как работает мини-разработка
...
Когда цель выполнена, ученик видит прогресс. Это превращает практику в понятную миссию.`
      },
      { id: "q_div_8", type: "divider" },
      {
        id: "q_lab_1",
        type: "text",
        textScale: "lg",
        body: `## Финальная сцена. Восстановить Архив Ирисов

Задача: собрать в Noda мини-модель, которая поможет Хранителю определить вид ириса по измерениям.

Это первая настоящая разработка: не просто прочитать, а собрать цикл ИИ своими руками.`
      },
      {
        id: "q_lab_studio",
        type: "studio",
        instruction:
          "Миссия: выбери tabular-датасет Iris, добавь блок обучения, запусти обучение и сделай предсказание для нового цветка. Посмотри на уверенность и подумай: можно ли доверять ответу модели?",
        studioPracticeKind: "empty",
        studioWorkspaceLevel: 1,
        goals: [
      ...
      },
      {
        id: "q_final_check",
        type: "checkpoint",
        question: "Что нужно проверить перед тем, как доверять модели?",
        answerMode: "multi",
        options: ["Качество на тестовой выборке", "Loss и ошибки", "Качество данных", "Цвет кнопки запуска"],
      ...
      },
      {
        ...
        kind: "image",
        url: "/api/coach/success.png",
        caption: "Хранитель: Архив Ирисов снова читается. Добро пожаловать в Noda."
      },
      {
        id: "q_final_2",
        type: "text",
        body: `## Итог первого урока

Ученики узнали:
- что такое ИИ;
- какие задачи решает ИИ;
- что такое данные, разметка, датасет и CSV;
- как работает train/test;
- что такое accuracy, confidence и loss;
- чем опасны недообучение и переобучение;
- как собрать первый цикл модели в Noda.

Главная мысль: **ИИ — это не магия. Это данные + модель + проверка + практика.**`
      }
    ],
    presentationPdfUrl: null,
    slides: [],
  ...
  await prisma.lessonTemplate.upsert({
    ...
    create: {
      ...
      description: "Полноценная игровая миссия по ИИ: теория + мини-лаборатория Noda.",
      moduleKey: "module_a",
      sortOrder: 0,
      starterPayload: EMPTY_SNAPSHOT,
